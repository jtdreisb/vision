...
if args.branch==1
    ...
    switch lower(args.mode)
        case {'affine', 'affine_notranslation', 'homography', 'homography_notranslation', 'homography_special', 'homography_special_notranslation'}
...
%             max_offset=0.05/max(size(input_image));
%             level=2;
%             candidate_matrix=cell(2, 2*args.branch_accuracy+1);
%             for i=1:2*args.branch_accuracy+1
%                 candidate_matrix{1, i}=eye(3);
%                 candidate_matrix{1, i}(3, 1)=-max_offset+(i-1)*max_offset/args.branch_accuracy;
%                 candidate_matrix{2, i}=eye(3);
%                 candidate_matrix{3, i}(3, 2)=-max_offset+(i-1)*max_offset/args.branch_accuracy;
%             end
        case {'euclidean', 'euclidean_notranslation'}
            max_rotation=args.branch_max_rotation;
            level=1;
